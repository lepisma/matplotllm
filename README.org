...
1. The iterative feedback could be provided in a better shell like way rather
   ...
   actually using context either in the request or via summarizing. This is not
   done yet.
...
4. Based on the output at every call during development, plot description
   usually needs a lot of iterations to get to what you want and also just to
   keep redraws consistent. This is obvious as writing specifications of
   /anything/ brings out many hidden unshared models between the parties involved.
   Data description, on the other hand, worked well once specified.
